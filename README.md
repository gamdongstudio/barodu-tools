...
2. 처음 설치할 때 Windows 보안 안내가 나타날 수 있습니다. **추가 정보 → 실행** 을 누르면 설치됩니다.
   (아직 코드 서명 인증서를 붙이지 않은 파일이라 이 창이 뜹니다)
3. 설치가 끝나면 화면 오른쪽 아래 시계 옆에 BARODU Tools 아이콘이 생깁니다.
4. BARODU PAGE MAKER 와 연결하려면 Chrome 에서 **"로컬 네트워크 접근"** 권한을 허용해주세요.
   (처음 연결할 때 Chrome 이 허용할지 묻습니다)

Windows 시작할 때 함께 켤지는 트레이 아이콘 메뉴에서 고를 수 있고, 고른 값은 다시 켜거나 업데이트해도 유지됩니다.
